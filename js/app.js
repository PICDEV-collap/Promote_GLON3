...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const btnConfirmAlreadyFriend = document.getElementById('btn-confirm-already-friend');
  const btnLineAddFriendNow = document.getElementById('btn-line-add-friend-now');
  ...
  async function checkLineMembershipAndNavigate(targetUrl, clickedBtn) {
    ...
    // ตรวจสอบความพร้อมของ LIFF SDK (ถ้ามี)
    ...
    if (typeof liff !== 'undefined' && isLiffReady && liff.isLoggedIn()) {
      ...
      if (userId) {
        ...
        if (checkResult && checkResult.isMember) {
          ...
          // ยังไม่ได้เพิ่มเพื่อน หรือบล็อก -> เปิดหน้าต่างให้กดเพิ่มเพื่อน
          showMemberModal();
          return;
        }
      }
  ...
  function parseOrderParamFromMessage(msg) {
    if (!msg) return '';
    const matches = [...msg.matchAll(/(\d{3})\s+(\d+)\s*ใบ/g)];
    if (matches.length > 0) {
      return matches.map(m => `${m[1]}:${m[2]}`).join(',');
    }
    const mSingle = msg.match(/(\d{3})/);
    if (mSingle) return `${mSingle[1]}:1`;
    return '';
  }
  ...
  function displayPredictionResult(pred) {
    ...
    // 0. Render Generative Dream Art 2.0
    const dreamArtImg = document.getElementById('dream-art-preview');
    if (dreamArtImg && typeof DreamArtGenerator !== 'undefined') {
      try {
        const artDataUrl = DreamArtGenerator.generateDreamArtDataUrl(pred);
        dreamArtImg.src = artDataUrl;
        window.currentDreamArtDataUrl = artDataUrl;
      } catch (artErr) {
        console.warn('Dream Art generation error:', artErr);
      }
    }

    const btnSaveDreamArt = document.getElementById('btn-save-dream-art');
    if (btnSaveDreamArt) {
      btnSaveDreamArt.onclick = async () => {
        const artUrl = window.currentDreamArtDataUrl || (dreamArtImg ? dreamArtImg.src : '');
        if (artUrl && typeof ImageSaver !== 'undefined') {
          await ImageSaver.saveImage({
            dataUrl: artUrl,
            filename: `GLO-N3-DreamArt-${pred.n3Direct || 'Lucky'}.png`,
            title: `ภาพนิมิตฝันเลขเด็ด ${pred.n3Direct || ''}`,
            text: `ภาพจำลองนิมิตฝัน AI เลขเด็ด 3 ตัวตรง: ${pred.n3Direct || ''}`
          });
        } else if (artUrl) {
          const a = document.createElement('a');
          a.href = artUrl;
          a.download = `GLO-N3-DreamArt-${pred.n3Direct || 'Lucky'}.png`;
    ...
    // 1. Setup Direct Web Table Order Links
    const btnOrderDreamTable = document.getElementById('btn-order-dream-table');
    if (btnOrderDreamTable) {
      btnOrderDreamTable.href = `order?numbers=${pred.n3Direct}&openExternalBrowser=1`;
    }

    const btnOrderDream6Pack = document.getElementById('btn-order-dream-6pack');
    if (btnOrderDream6Pack) {
      btnOrderDream6Pack.href = `order-6pack?numbers=${pred.n3Direct}&openExternalBrowser=1`;
    }

    // 2. Setup Lucky Package UI Presets
    ...
    // 4. Setup Package Click Handlers
    ...
    if (pkgBtnCombo) {
      pkgBtnCombo.onclick = () => {
        ...
        const paramItems = [`${pred.n3Direct}:1`];
        todsList.forEach(t => {
          orderItems.push(`${t} 1 ใบ`);
          paramItems.push(`${t}:1`);
        });
        const orderMsg = `สั่งซื้อ ${orderItems.join(', ')}`;
        const orderParam = paramItems.join(',');
        openLineOrder(dynamicLineOaId, orderMsg, orderParam);
      };
    }
    ...
    // 5. Update Floating Mobile Bottom Bar Action
    ...
    showToast('ทำนายฝัน สร้างภาพนิมิต และคำนวณเลข N3 สำเร็จแล้ว!');
  }
  ...
  if (btnModalSubmitOrder) {
    btnModalSubmitOrder.addEventListener('click', () => {
      ...
      openLineOrder(rawModalLine, cmd, orderParam);
    });
  }
  ...
  // -------------------------------------------------------------------------
  // 13. N3 Historical Probability Heatmap & Astrological Analytics Center
  // -------------------------------------------------------------------------
  let activeAnalyticsStats = null;

  function initAnalyticsCenter() {
    if (typeof N3AnalyticsEngine === 'undefined') return;

    N3AnalyticsEngine.loadData().then(draws => {
      activeAnalyticsStats = N3AnalyticsEngine.calculateStats(draws);
      
      // 1. Render Heatmap Matrix
      N3AnalyticsEngine.renderHeatmapDOM('n3-heatmap-container', activeAnalyticsStats);

      // 2. Render Astrological Day Cards
      renderAstroDayGrid(activeAnalyticsStats);

      // 3. Render Smart Recommended Combos
      renderSmartCombos(activeAnalyticsStats);
    });
  }

  function renderAstroDayGrid(stats) {
    const container = document.getElementById('astro-day-container');
    if (!container || !stats || !N3AnalyticsEngine.DAY_ASTRO_MATRIX) return;

    const days = N3AnalyticsEngine.DAY_ASTRO_MATRIX;
    let html = '';

    for (const [dayKey, dayData] of Object.entries(days)) {
      const isCurrentDay = (stats.astroInfo && stats.astroInfo.nameThai === dayData.nameThai);

      html += `
        <div class="astro-day-card" style="${isCurrentDay ? 'border-color: #ffd700; background: rgba(250, 204, 21, 0.1); box-shadow: 0 0 20px rgba(250, 204, 21, 0.25);' : ''}">
          <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 0.5rem;">
            <span style="font-weight: 800; font-size: 1.1rem; color: ${dayData.luckyColor};">${dayData.nameThai}</span>
            <span class="badge ${isCurrentDay ? 'badge-gold' : 'badge-glass'}" style="font-size: 0.72rem;">${dayData.planet}</span>
          </div>
          <div style="font-size: 0.8rem; color: var(--text-secondary); margin-bottom: 0.75rem;">
            ธาตุประจำวัน: <strong style="color: #fef08a;">${dayData.element}</strong>
          </div>
          <div style="margin: 0.75rem 0;">
            <div style="font-size: 0.78rem; color: var(--text-muted); margin-bottom: 0.35rem;">เลขกำลังวันเด่น:</div>
            <div>
              ${dayData.powerDigits.map(d => `<span class="astro-power-badge">${d}</span>`).join('')}
            </div>
          </div>
          <div style="margin-top: 0.75rem; padding-top: 0.65rem; border-top: 1px dashed rgba(255,255,255,0.1);">
            <div style="font-size: 0.75rem; color: var(--text-muted); margin-bottom: 0.3rem;">คู่เลขมงคล 2 ตัว:</div>
            <div style="display: flex; justify-content: center; gap: 0.35rem; flex-wrap: wrap;">
              ${dayData.luckyPairs.map(p => `
                <button type="button" class="tag-btn" onclick="N3AnalyticsEngine.dispatchToOrder('${p}')" style="padding: 0.25rem 0.6rem; font-size: 0.85rem; font-weight: 700; color: #fef08a;">
                  ${p}
                </button>
              `).join('')}
            </div>
  ...
  function renderSmartCombos(stats) {
    const container = document.getElementById('smart-combos-container');
    if (!container || !stats || !stats.smartCombos) return;

    let html = '';
    stats.smartCombos.forEach((combo, idx) => {
      html += `
        <div class="combo-num-pill" onclick="N3AnalyticsEngine.dispatchToOrder('${combo}')" title="คลิกเพื่อนำเลข ${combo} ไปสั่งซื้อในตาราง">
          <span>🎯 เลข ${combo}</span>
          <span style="font-size: 0.8rem; background: rgba(16,185,129,0.3); color: #6ee7b7; padding: 0.2rem 0.5rem; border-radius: 9999px;">
            🛒 สั่งซื้อ
          </span>
        </div>
      `;
